...
A Drive method that accepts content has an upload twin. `drive.files.upload(media=..., name=...)` sends bytes, a path, or a file-like with the same metadata `files.create` takes; `drive.files.update_media(file_id=..., media=...)` replaces a file's content. Uploads use Google's resumable protocol: a path streams from disk, and the only size limit is Drive's own.
